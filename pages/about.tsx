...
const CollaborateInfo : CollaborateData[] = [
	{
		...
		Description: "Budgie Desktop and all of our organization’s source code (including this site!) is entirely open source and developed on GitHub. We leverage GitHub for tracking issues and desired improvements in our software or our organization, as well as GitHub’s discussion forums for long-form public discussions.",
		Image: GitHub(),
	},
	{
		Alt: "Matrix",
					...
					<Box display="inline-flex" justifyContent="space-between" flexWrap="wrap">
						{
							Teams.map((team) => {
								return (
									<Team
										key={`Team-${team.Name.replaceAll(" ", "-")}`}
										stackSpacing={AboutPageSpacing}
										team={team}
									/>
								)
							})
							...
							CollaborateInfo.map((info) => {
								...
								return (
									<Stack
										...
										key={`Collaborate-${Alt}`}
									>
										<Box alignItems="top" display="inline-flex" sx={{ "& > svg": { maxWidth: "100%"} }} justifyContent="center" minWidth={200} width={200}>
											{ Image }
										</Box>
										<Stack
											alignItems="flex-start"
