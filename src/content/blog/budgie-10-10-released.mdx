...
Beyond our recommendation default of `labwc`, the move to Wayland represents a fundamental architectural shift. This shift to a protocol-first architecture represents a major milestone: it decouples the desktop from a specific window manager (budgie-wm, which in turn relied on our Magpie fork of Mutter) and makes Budgie truly compositor-agnostic, opening the door for experimentation with alternative compositors beyond our primary recommendations.
...
Budgie Desktop View supports Wayland, leveraging `layer-shell` to ensure it remains positioned behind windows, Raven, and the panel.
