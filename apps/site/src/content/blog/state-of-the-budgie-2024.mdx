...
Neal started an internal repo to centralize the Wayland session files of various compositors, starting off with a couple sessions (Magpie v1 and KWin) so I could start testing against them.
...
Neal added a [labwc](https://labwc.github.io/) session to our Wayland sessions and we began actively dogfooding under labwc instead of KWin, as it supports all the wlr Wayland protocols we need.
...
TODO: JOSH VIDEO HERE FROM CAMPBELL
...
TODO: JOSH VIDEO HERE FROM DAVID
...
As a last bit of fun, I tested Budgie Desktop under [Miriway](https://github.com/Miriway/Miriway/) thanks to Neal putting a Miriway compositor session file in our internal sessions repo. Miriway doesn't support all the protocols we need but maybe one day it will and Budgie users will have yet another compositor they can choose from! The more the merrier.
